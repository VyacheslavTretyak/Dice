function prepareGame(){
	...
	$("#gameMoney").html(getPage("menuGame"));	
	...
	$("#bSlideGameMenu").show();	
	$("#bSlideChat").show();	
	$("#bSlideChat").click(function(){		
		$("#divChatGame").animate({left: 0}, 1000);	
		$("#bSlideChat").hide();
		$("#bArrowLeftGame").show();
	});	
	$("#bArrowLeftGame").click(function(){		
		$("#divChatGame").animate({left: -1000}, 1000);	
		$("#bArrowLeftGame").hide();
		$("#bSlideChat").show();
	});	
	$("#bSlideGameMenu").click(function(){		
		$("#menuGame").animate({left: 450}, 1000);	
		$("#bSlideGameMenu").hide();
		$("#bArrowRightGame").show();
	});	
	$("#bArrowRightGame").click(function(){		
		$("#menuGame").animate({left: 1000}, 1000);
		$("#bSlideGameMenu").show();
...
function updateGame(){	
	...
	function success(res){
		...
		for(var i=0;i<r.chat.length;i++){			
			...
			if((i==0) && (r.chat[i][3] > getCookie("lastMess"))){
				if(getCookie("lastMess") > 0){
					$("#newMess").fadeIn();
					$("#newMess").html(r.chat[i][2]);
				}
				setTimeout(function(){
					$("#newMess").fadeOut();
				}, 3000);
				setCookie("lastMess", r.chat[i][3]);
			}
...
function anim(val,  x){	
	if(x == 0)
		return;	
	$("#value").stop();
	$("#value").css({opacity: 1, top: "0px", left:"0px", fontSize: "650%"});	
	$("#value").offset({top: x.top-50, left: x.left});
	$("#value").html(val);
	$("#value").animate({
		opacity: 0,
		top: "-=70",
